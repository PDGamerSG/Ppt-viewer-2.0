...
const LO_PROFILE = 'file:///' + path.join(os.tmpdir(), 'pptviewer-lo-profile').replace(/\\/g, '/');
...
function getCachedPdf(filePath) {
  ...
  if (fs.existsSync(cachedPath)) {
    // Touch so LRU pruning keeps recently-used entries
    const now = new Date();
    try { fs.utimesSync(cachedPath, now, now); } catch (_) {}
    return cachedPath;
  }
  return null;
}

// Prune stale cache entries on startup.  Keeps the cache useful across
// sessions without letting it grow unbounded.
//   - Drop entries older than MAX_AGE_MS
//   - Cap total PDFs to MAX_ENTRIES, evicting least-recently-modified first
...
function pruneCache() {
  try {
    ...
    const now = Date.now();
    const entries = fs.readdirSync(CACHE_DIR).map((name) => {
      const full = path.join(CACHE_DIR, name);
      try {
        const stat = fs.statSync(full);
        return { full, name, mtimeMs: stat.mtimeMs, isPdf: name.toLowerCase().endsWith('.pdf') };
      } catch (_) {
        return null;
      }
    }).filter(Boolean);

    // Remove any leftover non-PDF temp inputs from prior crashed runs
    ...
    const pdfs = entries.filter((e) => e.isPdf);
    ...
    // Size-based eviction: keep newest MAX_ENTRIES
    const remaining = pdfs.filter((e) => !e._removed).sort((a, b) => b.mtimeMs - a.mtimeMs);
    for (let i = MAX_ENTRIES; i < remaining.length; i++) {
      try { fs.unlinkSync(remaining[i].full); } catch (_) {}
    }
  } catch (_) {}
}
...
// Pre-warm LibreOffice — spawns a real headless invocation with the same
// profile and flags used for conversion so the OS caches soffice binary,
// DLLs, JVM, and the user profile is initialized.  Much more effective
// than --version which barely loads anything.
//
// The prewarm is chained into the conversion queue so a real conversion
// never races with it — but it auto-kills after 8s if it hasn't finished.
let prewarmProc = null;

function prewarmLibreOffice(sofficePath) {
  if (!sofficePath) return;
  try {
    prewarmProc = spawn(sofficePath, [
      '--headless',
      '--nologo',
      '--norestore',
      '--nofirststartwizard',
    ...
    const prewarmDone = new Promise((resolve) => {
      let resolved = false;
      const done = () => { if (!resolved) { resolved = true; prewarmProc = null; resolve(); } };
      prewarmProc.on('close', done);
      prewarmProc.on('error', done);
      // Don't let prewarm block conversions for more than 8s
      setTimeout(() => {
        if (prewarmProc) {
          try { prewarmProc.kill(); } catch (_) {}
        }
        done();
      }, 8000);
    });
